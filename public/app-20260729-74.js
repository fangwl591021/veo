...
async function home() {
  ...
  layout(`<section class="member-portal ak-home-banner"><button type="button" class="portal-profile" data-home-action="profile" aria-label="開啟會員資料">${avatar()}<strong>${esc(greeting)}</strong><small>${dateText}</small></button><button type="button" class="portal-primary" data-home-action="wallet"><div><span>商脈點數</span><strong>${format(wallet.balance)}</strong></div></button><button type="button" class="portal-primary" data-home-action="share"><div><span>專屬分享</span></div></button></section><section class="ak-dashboard">
