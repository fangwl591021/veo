...
function destinySourceCard(label, value, detail, status = "ready") {
  const statusLabel = status === "ready" ? "已分析" : status === "ai" ? "AI 整合" : "待補資料";
  return `<article class="destiny-source-card ${status}"><header><strong>${esc(label)}</strong><span>${statusLabel}</span></header><b>${esc(value)}</b><p>${esc(detail)}</p></article>`;
}
function destinySourcesMarkup(fortune) {
  ...
  return [
    ...
    destinySourceCard("MBTI","待完成測驗","不可僅憑生日推測人格類型。","locked"),
    destinySourceCard("DISC","待完成測驗","完成行為風格題目後才能納入。","locked"),
    destinySourceCard("九型人格","待完成測驗","需要動機與壓力反應資料。","locked"),
    destinySourceCard("Big Five","待完成測驗","需要五大人格量表結果。","locked"),
...
async function loadDestinyAiAnalysis(root = document) {
  ...
  try {
    ...
    target.innerHTML = `<b>AI 分析準備中</b><p>系統正在整合姓名、生日與會員資料；稍後再次開啟即可查看，不影響今日導航卡。</p>`;
  } catch (error) {
    target.className = "destiny-ai-content failed";
    target.innerHTML = `<b>AI 分析暫時無法載入</b><p>生日可計算的分析與今日導航卡仍可正常使用。</p>`;
  }
}
