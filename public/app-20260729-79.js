...
async function loadAkaffitYoutube() {
  ...
  try {
    ...
    panel.innerHTML = `<header class="ak-youtube-head"><div><small>OFFICIAL YOUTUBE</small><h2>${esc(data.channel?.title || "Akaffit 咖啡")}</h2></div><a href="https://www.youtube.com/@akaffit" target="_blank" rel="noopener noreferrer">前往官方頻道 ↗</a></header>${videos.length ? `<div class="ak-youtube-grid">${videos.map((video) => `<a class="ak-youtube-card" href="${esc(video.url)}" target="_blank" rel="noopener noreferrer"><span><img src="${esc(video.thumbnailUrl)}" alt="" loading="lazy"><i aria-hidden="true">▶</i></span><strong>${esc(video.title)}</strong></a>`).join("")}</div>` : '<p class="ak-youtube-empty">目前尚無公開影片。</p>'}`;
  } catch (error) {
...
async function home() {
  ...
  layout(`<section class="member-portal ak-home-banner"><button type="button" class="ak-home-avatar" data-home-action="profile" aria-label="開啟會員資料">${avatar()}</button><button type="button" class="portal-primary" data-home-action="wallet"><div><span>有點開心</span><strong>${format(wallet.balance)}</strong></div></button><button type="button" class="portal-primary ak-zodiac-entry" data-home-action="zodiacPopup"><span class="ak-zodiac-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 2.5l2.1 5.2 5.4.4-4.1 3.5 1.3 5.3-4.7-2.8-4.7 2.8 1.3-5.3-4.1-3.5 5.4-.4z"/></svg></span><span>星座運勢</span></button><button type="button" class="portal-primary ak-share-entry" data-home-action="share"><span class="ak-share-qr-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM15 14h2v2h-2zM19 14h2v4h-2zM14 18h3v3h-3zM19 20h2v1h-2z"/></svg></span><span>專屬分享</span></button></section><section class="ak-dashboard">
