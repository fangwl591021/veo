    ...
    <section class="ak-golden-journey" aria-label="醇金之旅線上簡報" data-golden-journey>
      <div class="ak-golden-stage">
        ...
        <div class="ak-golden-track" data-golden-track tabindex="0" aria-label="醇金之旅投影片，可左右滑動">
          ${goldenJourneySlideUrls.map((url, index) => `<figure class="ak-golden-slide" data-golden-slide="${index}" aria-label="第 ${index + 1} 張，共 ${goldenJourneySlideUrls.length} 張"><img src="${url}" alt="醇金之旅第 ${index + 1} 張投影片" loading="${index === 0 ? "eager" : "lazy"}" decoding="async"></figure>`).join("")}
        </div>
        <button type="button" class="ak-golden-arrow is-next" data-golden-next aria-label="下一張投影片">›</button>
      </div>
      <footer class="ak-golden-footer"><strong data-golden-counter>1 / ${goldenJourneySlideUrls.length}</strong><span>左右滑動瀏覽・每 10 秒自動播放</span></footer>
    </section>
  </div>`;
}
...
function initGoldenJourney() {
  ...
  track.addEventListener("pointercancel", restartAutoPlay, { passive:true });
