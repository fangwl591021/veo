...
function layout(body) {
  ...
  const headerActions = `<div class="feature-header-actions">${cardCollectionAction}<button class="feature-header-action feature-home-action" data-home-action="home" aria-label="返回首頁"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 11 8-7 8 7M6.5 9.5V20h11V9.5M9.5 20v-6h5v6"/></svg><span>返回首頁</span></button></div>`;
...
async function renderLogin() {
  const isLogin = phoneAuthMode === "login";
  $("#app").innerHTML = `<section class="ak-register"><div class="ak-register-wordmark">A-KAFFIT TEAM</div><h1>會員登入或註冊</h1><p>已有會員請先登入；第一次使用才建立新會員，避免重複註冊。</p><div class="ak-auth-tabs" role="tablist" aria-label="會員登入與註冊"><button type="button" role="tab" aria-selected="${isLogin}" class="${isLogin ? "active" : ""}" data-phone-auth-mode="login">會員登入</button><button type="button" role="tab" aria-selected="${!isLogin}" class="${!isLogin ? "active" : ""}" data-phone-auth-mode="register">新會員註冊</button></div><div class="ak-auth-mode-note">${isLogin ? "輸入已註冊的手機與生日密碼。" : "確認尚未註冊後，再建立新的會員資料。"}</div><label>手機<input id="registerPhone" type="tel" inputmode="tel" autocomplete="tel" placeholder="0912345678" maxlength="20"></label><label>生日密碼（民國年月日）<input id="registerBirthday" type="text" inputmode="numeric" autocomplete="bday" placeholder="例如 591021、390305" pattern="[0-9]{6,7}" maxlength="7"></label><button class="btn" id="phoneAuthSubmit">${isLogin ? "登入會員中心" : "建立新會員"}</button><em>${isLogin ? "查無會員時，系統不會自動建立帳號。" : "已註冊的手機與生日不會再次建立帳號。"}</em></section>`;
...
async function showContactEditor(card) {
  ...
  if (view === "edit") panel = `<form id="collectionCardForm" class="business-card-form"><div class="collection-edit-sections full">${collectionEditSection("行業分類","主行業與次行業",collectionIndustryForm(card))}${collectionEditSection("聯絡資料","姓名、公司、電話與地址",collectionForm(card,"contact"))}${collectionEditSection("業務暖身線索","生日、性別與 FORMHD",collectionCrmProfileForm(card))}</div><p class="collection-verification-note full">儲存前會二次查核姓名、公司、電話、Email、網站與地址；任何已填欄位無法確認時都不會儲存。</p><button class="btn full" type="submit">查核並儲存</button><button class="btn danger full" type="button" id="deleteContact">刪除名片</button></form>`;
