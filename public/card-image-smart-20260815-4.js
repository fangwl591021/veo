...
function paddedRange(start, end, limit, padding) {
  const size=Math.max(1,end-start),target=Math.min(limit,size*(1+padding*2));
  let paddedStart=(start+end-target)/2,paddedEnd=paddedStart+target;
  if(paddedStart<0){paddedEnd-=paddedStart;paddedStart=0;}
  if(paddedEnd>limit){paddedStart-=paddedEnd-limit;paddedEnd=limit;}
  return [clamp(paddedStart,0,limit),clamp(paddedEnd,0,limit)];
}

export function expandCardQuad(points, width, height, padding = 0.10) {
  if(!Array.isArray(points)||points.length!==4||!width||!height)return points;
  const xs=points.map((point)=>point.x),ys=points.map((point)=>point.y);
  const [left,right]=paddedRange(Math.min(...xs),Math.max(...xs),width,padding);
  const [top,bottom]=paddedRange(Math.min(...ys),Math.max(...ys),height,padding);
...
export async function processBusinessCardImage(file) {
  ...
  try{
    ...
    if(!found)return {file:null,metadata:{processingVersion:"card-image-v3",detection:{detected:false,confidence:0},quality:qualityScores(imageData,0),processing:{perspectiveCorrected:false,cropped:false,rotated:false,lightingEnhanced:false,manualCorrection:false},corners:[],warning:"找不到清楚的名片邊界"}};
    const quality=qualityScores(imageData,found.coverage);const expandedPoints=expandCardQuad(found.points,detection.width,detection.height);const normalized=expandedPoints.map((point)=>({x:point.x/detection.width,y:point.y/detection.height}));
    const orientation=found.horizontal>=found.vertical ? "landscape" : "portrait";
    const metadata={processingVersion:"card-image-v3",original:{width:bitmap.width,height:bitmap.height},detection:{detected:true,confidence:found.confidence,boundaryScore:found.boundaryScore},card:{orientation,rotation:0},quality,processing:{perspectiveCorrected:true,cropped:true,rotated:false,lightingEnhanced:false,manualCorrection:false},corners:normalized,warning:found.confidence<CARD_IMAGE_THRESHOLDS.recommendedConfidence ? "名片邊界信心較低，請在辨識結果頁確認內容" : ""};
    ...
    const expandedAspect=Math.max(horizontal,vertical)/Math.max(1,Math.min(horizontal,vertical));const longSide=Math.min(1600,Math.max(1000,Math.round(Math.max(horizontal,vertical))));const shortSide=Math.round(longSide/Math.max(1.25,expandedAspect));
