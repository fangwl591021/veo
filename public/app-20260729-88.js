...
async function taskCenter(){
  ...
  try{
    ...
    $("#saveTaskChannels").onclick=async()=>{const button=$("#saveTaskChannels");try{await withActionFeedback(button,()=>api("/v1/tasks/channels",{method:"PUT",body:JSON.stringify({telegramEnabled:$("#taskTelegramEnabled").checked,telegramChatId:$("#taskTelegramChatId").value})}),{busy:"儲存中…",success:"已儲存"})}catch(error){alert(error.message||"推播設定失敗")}};
