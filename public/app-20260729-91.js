...
function taskChannelMarkup(channel,capabilities){
  const telegramReady=channel.telegramEnabled&&capabilities.telegramConfigured;
  const status=telegramReady?"Telegram 已啟用":channel.telegramEnabled?"Telegram 憑證未設定":"Telegram 未啟用";
  return `<section class="task-channel-card"><details><summary><span class="task-channel-title"><b>推播設定</b><small>任務提醒選配</small></span><span class="task-channel-state ${telegramReady?"is-ready":""}">${esc(status)}</span><i aria-hidden="true"></i></summary><div class="task-channel-grid"><div class="task-channel-heading"><div><strong>Telegram 任務提醒</strong><small>建立或手動發送任務提醒時，傳送到指定帳號。</small></div><button id="taskTelegramHelp" type="button" aria-label="查看 Telegram 設定說明">?</button></div><label class="task-channel-field">Telegram Chat ID<input id="taskTelegramChatId" inputmode="numeric" autocomplete="off" value="${esc(channel.telegramChatId||"")}" placeholder="例如：123456789"></label><label class="task-channel-toggle"><input id="taskTelegramEnabled" type="checkbox" ${channel.telegramEnabled?"checked":""}><span>使用 Telegram Push</span></label><button id="saveTaskChannels" type="button">儲存推播設定</button><small>未啟用推播時，任務、CRM 回報與 AI 下一步仍可正常使用。</small></div></details></section><dialog id="taskTelegramHelpDialog" class="task-help-dialog"><form method="dialog"><header><strong>Telegram Chat ID 設定</strong><button type="submit" aria-label="關閉說明">×</button></header><ol><li>先開啟系統提供的 Telegram Bot，按下 Start 或傳送 <b>/start</b>。</li><li>取得自己的數字 Chat ID，貼到推播設定欄位。</li><li>勾選「使用 Telegram Push」並儲存。</li><li>建立任務後，可在任務卡片按「發送提醒」確認通知。</li></ol><p>目前沒有串接 LINE OA；LINE 推播能力保留在系統中，但會員畫面先隱藏。</p><button class="btn" type="submit">我知道了</button></form></dialog>`;
}
...
async function taskCenter(){
  ...
  try{
    ...
    $("#saveTaskChannels").onclick=async()=>{const button=$("#saveTaskChannels");try{await withActionFeedback(button,()=>api("/v1/tasks/channels",{method:"PUT",body:JSON.stringify({lineEnabled:channel.lineEnabled===true,telegramEnabled:$("#taskTelegramEnabled").checked,telegramChatId:$("#taskTelegramChatId").value})}),{busy:"儲存中…",success:"已儲存"})}catch(error){alert(error.message||"推播設定失敗")}};$("#taskTelegramHelp").onclick=()=>$("#taskTelegramHelpDialog").showModal();
...
function homeGreeting(date = new Date()) {
  ...
  return hour >= 5 && hour < 12 ? "早安" : hour >= 12 && hour < 18 ? "午安" : "晚上好";
}
...
async function home() {
  ...
  const memberName = state.member?.displayName || state.member?.fullName || "會員";
  ...
  layout(`<section class="ak-dashboard">
    ...
    <section class="ak-home-content">
      <div class="ak-frozen-nav">
        <div class="ak-feature-grid">
          ...
          <button data-home-action="daily">${homeToolIcons.daily}<span>每日簽到</span></button>
    ...
    <nav class="ak-bottom-nav" aria-label="主要導覽"><button class="active" data-home-action="home"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 11 8-6.5 8 6.5v9H4zM9 20v-6h6v6"/></svg><span>首頁</span></button><button data-home-action="cardCollection"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="8" r="3"/><circle cx="17" cy="8.5" r="2.5"/><path d="M2.8 19c.5-3.6 2.2-5.4 5.2-5.4s4.7 1.8 5.2 5.4M13.5 14c2.9-.8 5.8.9 6.5 4.5"/></svg><span>人脈</span></button><button data-home-action="smartMatch"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 18 9 12l4 3 7-9M15 6h5v5"/></svg><span>洞察</span></button><button data-home-action="profile"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path d="M5 20c.7-4.3 3-6.5 7-6.5s6.3 2.2 7 6.5"/></svg><span>我的</span></button></nav>
