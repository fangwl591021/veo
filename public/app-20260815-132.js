...
const pointEventLabel = { member_joined:"加入會員", registration_completed:"完成註冊", share_referral:"分享邀約成功", daily_ad_checkin:"每日簽到", course_registered:"課程報名", attendance_verified:"課程簽到", calendar_checkin_reward:"行事曆簽到點數", referral_attendance_reward:"所屬會員完成獎勵", task_completed:"完成任務", card_collection_reward:"掃描名片", card_collection_reward_reversal:"刪除名片扣回" };
...
const smartCheckinReason = { no_active_session:"目前沒有可報到的活動，請確認報到時間。", registration_required:"尚未報名此場活動，無法完成簽到。", session_unavailable:"此場活動目前無法簽到。", insufficient_points:"點數餘額不足，無法完成簽到。" };
...
function showCollectionReview(eventId, card, confidence) {
  ...
  $("#saveScannedCard").onclick=async()=>{const button=$("#saveScannedCard");try{await withActionFeedback(button,async()=>{
    ...
    else if(result.body.reward?.status==="completed")alert("收藏成功，已贈送 10 點數。");
    else alert("收藏成功，10 點數正在入帳，系統會自動重試。");
