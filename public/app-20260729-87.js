...
async function taskCenter(){
  ...
  try{
    ...
    const tasks=Array.isArray(data.tasks)?data.tasks:[],events=Array.isArray(data.events)?data.events:[],deliveries=Array.isArray(data.deliveries)?data.deliveries:[],contacts=Array.isArray(data.contacts)?data.contacts:[];
    ...
    layout(`<section class="task-engine-intro"><small>TASK ENGINE</small><h2>AI 商務導航</h2><p>提醒 → 執行 → 回報 → 分析 → 下一步</p></section><section class="card task-create"><header><div><small>建立任務</small><h2>今天要推進什麼？</h2></div><button id="toggleTaskForm" type="button">＋ 新增</button></header><form id="taskCreateForm" hidden><label>任務名稱<input id="taskTitle" maxlength="120" required placeholder="例如：回訪王小姐確認合作需求"></label><label>關聯名片<select id="taskContact"><option value="">不指定聯絡人</option>${contactOptions}</select></label><div class="task-form-row"><label>期限<input id="taskDueAt" type="datetime-local" value="${taskLocalValue(tomorrow)}" required></label><label>優先順序<select id="taskPriority"><option value="normal">一般</option><option value="high">高</option><option value="low">低</option></select></label></div><label>說明<textarea id="taskDescription" rows="3" maxlength="2000" placeholder="成功標準、要確認的內容…"></textarea></label><button class="btn" id="taskCreateButton" type="submit">建立並發送提醒</button></form></section><section class="task-channel-card"><details><summary>推播設定 <span>${channel.lineLinked?(capabilities.lineConfigured?"LINE Push 可用":"LINE 憑證未設定"):"LINE 未連結"}・${channel.telegramEnabled?(capabilities.telegramConfigured?"Telegram 可用":"Telegram 憑證未設定"):"Telegram 未啟用"}</span></summary><div class="task-channel-grid"><label><input id="taskLineEnabled" type="checkbox" ${channel.lineEnabled!==false?"checked":""}> 使用 LINE Push</label><label>Telegram Chat ID<input id="taskTelegramChatId" value="${esc(channel.telegramChatId||"")}" placeholder="輸入 Chat ID 後啟用"></label><label><input id="taskTelegramEnabled" type="checkbox" ${channel.telegramEnabled?"checked":""}> 使用 Telegram Push</label><button id="saveTaskChannels" type="button">儲存推播設定</button><small>未設定推播憑證時不影響任務與 CRM 回報。</small></div></details></section><section class="task-engine-section"><header><div><small>執行中</small><h2>待辦任務</h2></div><b>${active.length}</b></header><div class="task-engine-list">${active.length?active.map((task)=>taskCardMarkup(task,events,deliveries)).join(""):'<div class="task-empty">目前沒有待辦任務。建立一筆，讓 AI 開始協助導航。</div>'}</div></section>${closed.length?`<section class="task-engine-section closed"><header><div><small>已回報</small><h2>CRM 任務紀錄</h2></div><b>${closed.length}</b></header><div class="task-engine-list">${closed.map((task)=>taskCardMarkup(task,events,deliveries)).join("")}</div></section>`:""}`);
    $("#toggleTaskForm").onclick=()=>{$("#taskCreateForm").hidden=!$("#taskCreateForm").hidden;if(!$("#taskCreateForm").hidden)$("#taskTitle").focus()};
    ...
    $("#saveTaskChannels").onclick=async()=>{const button=$("#saveTaskChannels");try{await withActionFeedback(button,()=>api("/v1/tasks/channels",{method:"PUT",body:JSON.stringify({lineEnabled:$("#taskLineEnabled").checked,telegramEnabled:$("#taskTelegramEnabled").checked,telegramChatId:$("#taskTelegramChatId").value})}),{busy:"儲存中…",success:"已儲存"})}catch(error){alert(error.message||"推播設定失敗")}};
...
async function home() {
  const result = await Promise.allSettled([api("/v1/points/wallet")]);
  const wallet = result[0].status === "fulfilled" ? result[0].value.wallet : { balance:0 };
  layout(`<section class="member-portal ak-home-banner"><button type="button" class="ak-home-avatar" data-home-action="profile" aria-label="開啟會員資料">${avatar()}<span class="ak-home-label">會員專區</span></button><button type="button" class="portal-primary" data-home-action="wallet"><div><strong>${format(wallet.balance)}</strong><span>有點開心</span></div></button><button type="button" class="portal-primary ak-zodiac-entry" data-home-action="zodiacPopup"><span class="ak-zodiac-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 2.5l2.1 5.2 5.4.4-4.1 3.5 1.3 5.3-4.7-2.8-4.7 2.8 1.3-5.3-4.1-3.5 5.4-.4z"/></svg></span><span class="ak-home-label">星座運勢</span></button><button type="button" class="portal-primary ak-share-entry" data-home-action="share"><span class="ak-share-qr-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM15 14h2v2h-2zM19 14h2v4h-2zM14 18h3v3h-3zM19 20h2v1h-2z"/></svg></span><span class="ak-home-label">專屬分享</span></button></section><section class="ak-dashboard">
    <section class="ak-home-content">
      <div class="ak-frozen-nav">
        <div class="ak-feature-grid">
          <button data-home-action="cardCollection"><span>名片收藏</span></button>
          <button data-home-action="daily"><span>每日簽到</span></button>
          <button data-home-action="smartMatch"><span>智能配對</span></button>
          <button data-home-action="calendar"><span>個人行程</span></button>
          <button data-home-action="tasks"><span>AI 任務</span></button>
        ...
        <div class="ak-content-tabs" role="tablist" aria-label="品牌內容切換">
          <button type="button" role="tab" aria-selected="false" data-content-view="youtube">YouTube</button>
          <button type="button" role="tab" aria-selected="false" data-content-view="facebook">Facebook</button>
          <button type="button" role="tab" aria-selected="false" data-content-view="instagram">Instagram</button>
          <button type="button" role="tab" aria-selected="true" class="active" data-content-view="official">官方網站</button>
          <button type="button" role="tab" aria-selected="false" data-content-view="academy">咖啡學院</button>
        </div>
      </div>
      <section class="ak-youtube-panel ak-content-panel hidden" data-content-panel="youtube" aria-label="A’kaffit YouTube 頻道"></section>
      <section class="ak-facebook-panel ak-content-panel hidden" data-content-panel="facebook" aria-label="A’kaffit Facebook 粉絲專頁">
        <iframe class="ak-facebook-frame" title="A’kaffit Facebook 粉絲專頁" src="https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Fprofile.php%3Fid%3D61565353201161&amp;tabs=timeline&amp;width=500&amp;height=900&amp;small_header=true&amp;adapt_container_width=true&amp;hide_cover=false&amp;show_facepile=false" loading="lazy" scrolling="yes" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>
      </section>
      <section class="ak-instagram-panel ak-content-panel hidden" data-content-panel="instagram" aria-label="A’kaffit Instagram">
        <iframe class="ak-instagram-frame" title="A’kaffit Instagram" src="https://www.instagram.com/akaffit/embed/" loading="lazy" scrolling="yes" frameborder="0" allowtransparency="true"></iframe>
        <div class="ak-instagram-more"><p>Instagram 官方預覽顯示近期貼文</p><a href="https://www.instagram.com/akaffit/" target="_blank" rel="noopener noreferrer">查看更多 Instagram 貼文 ↗</a></div>
      </section>
      <section class="ak-official-import ak-content-panel" data-content-panel="official" aria-label="A’kaffit 官方網站">
        <div class="ak-official-import-loading">A’kaffit 官網載入中…</div>
        <iframe class="ak-official-import-frame" title="A’kaffit 官方網站" src="/akaffit-official" loading="eager"></iframe>
      </section>
      <section class="ak-academy-panel ak-content-panel hidden" data-content-panel="academy" aria-label="咖啡學院">
        <div><small>COFFEE ACADEMY</small><h2>咖啡學院</h2><p>內容建置中</p></div>
      </section>
    </section>
...
function aiCardCrmSection(card) {
  ...
  if(status==="queued" || status==="processing")return `<section class="ai-card-crm-result pending"><h3>AI 智慧名片 CRM</h3><p>正在補全公司、社群、新聞與知識卡，完成後會自動建立第一個行程任務。</p><button class="btn alt" type="button" data-retry-ai-crm>立即重新補全</button></section>`;
  if(status==="failed")return `<section class="ai-card-crm-result pending"><h3>AI 智慧名片 CRM</h3><p>公司資料補全暫時失敗：${esc(crm.error || "系統將自動重試")}。</p><button class="btn alt" type="button" data-retry-ai-crm>重新補全</button></section>`;
  if(status!=="ready")return `<section class="ai-card-crm-result pending"><h3>AI 智慧名片 CRM</h3><p>這張舊名片尚未完成公司與社群資料補全。</p><button class="btn alt" type="button" data-retry-ai-crm>開始補全</button></section>`;
...
async function showContactEditor(card) {
  ...
  $("[data-retry-ai-crm]")?.addEventListener("click", async (event) => {
    try {
      await withActionFeedback(event.currentTarget, () => api(`/v1/card-collection/${encodeURIComponent(card.id)}/recalculate-ai-crm`, { method:"POST", body:"{}" }), { busy:"補全排程中…", success:"已開始補全" });
